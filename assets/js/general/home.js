...
const home = () => {
  ...
  const homeScrolling = () => {
    ...
    const smoothScroll = (target, duration = 200, callback = null) => {
      ...
      let direction = offset > currentOffset ? 1 : -1
      let currentTime = 0
    ...
    window.addEventListener('scroll', e => {
      let fromTop = document.scrollingElement.scrollTop + 10
      ...
      if (lastId !== id) {
        ...
        adjustMain(id)
        menuItems.forEach(
          mItem => mItem.getAttribute('href') === `#${id}`
            ? mItem.parentElement.classList.add('current')
            : mItem.parentElement.classList.remove('current')
        )
      }
    })
